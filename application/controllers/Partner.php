...
//		$session=$this->param['session'];echo_r($session);echo_r($array);exit;
	...
	function promotion($type=null){
		...
		$contents=array('partner_promotion');
...
//=============URL REGISTER
		$res= _localApi('account','list_simple',array($this->param['userlogin']['email']));
//echo_r($res);
		$account =   isset($res['data'])?$res['data']:array();
		$total_members=0;
		...
		$this->param['outerJS'][]=site_url('partner/js/partner_promotion').'?account_id='.$account_id.'&name='.$name ;
...
//		die( json_encode($post));
		...
		$arr=array('html'=>'<div id="salmamarkets_banner_'.$set.'_'.$dimension.'">'.$txt.'</div>');
