    ...
    public function uploads($warn = 0) {
        ...
        if ($this->input->post('rand')) {
            ...
            if ($files['size'] > 550000) {
                ...
                redirect(site_url('member/uploads/' . $rand));
                exit();
            }
...
//-----------EMAIL
                ...
                echo 'not valid';
                redirect(site_url("depan/editPassword"));
            }
    ...
    private function withdrawal_page($status = null) {
        ...
        if ($this->input->post('orderWidtdrawal')) {
            ...
            if ($uDetail['users']['u_mastercode'] != $post0['mastercode']) {
                ...
                $this->session->set_flashdata('notif', $notif);
                //===================SMS TENTANG MASALAH MASTERCODE
                $email = $uDetail['email'];
                $phone = $this->users_model->phone_by_email( $email );
                $sms_text   =   "Withdrawal Warning";
                $sms_text   .=   "\nThis message send Because You or Someone entering Mastercode into WD Form.\n";
                $sms_text   .=   "\nYour Mastercode: ".$uDetail['users']['u_mastercode'];
                $sms_text   .="\nPlease Update Your Mastercode, after using this\n";
                //====================SMS===================
                $params=array(
                   'debug'=>true,
                    'number'=>$phone,
                    'message'=>$sms_text."Sincerely, System Salmamarkets.",
    ...
    private function checkLogin_3() {
        ...
        if (isset($detail['name']) && trim($detail['name']) != '') {
            ...
            if ($page == 'edit' || $page == 'edit_master_password') {
                ...
                js_goto(site_url('member/edit') . '?r=input_your_name');
                exit;
            }
        }
        $session = $this->param['session'];
