...
***/
	...
	public function edit($warn=0){
		...
		//========update detail forex
			...
			redirect(base_url('member/edit')."?rand=$rand");
			/*
	...
	public function editpassword(){
		...
		//	echo '<pre>'.print_r($userlogin,1);exit();
			...
			//echo_r($p);
			//echo 'not valid';
			redirect(base_url("member/editPassword"));
			exit;
		}
	...
	public function edit_master_password(){
		...
		//	echo_r($post0);echo_r($userlogin);exit;
			...
			redirect(base_url("member/edit_master_password?success=".date("Ymd") ));
			exit;
		}
		...
		if(isset($userlogin['users']['u_mastercode'] )&&trim($userlogin['users']['u_mastercode'])!='' ){
			...
			redirect(base_url("member/edit_master_password?reload=".date("Ymd") ));
			exit;
		}
	}
...
//==================
	...
	public function deposit($status='none'){
		...
		//	echo_r($this->param);
			...
			redirect(base_url('member/deposit/done/'.rand(2100,8999) ),true);
...
*/
	...
	logCreate('member |withdrawal_page |start');
		...
		//	die($message);
			...
			$this->session->set_flashdata('notif',$notif);
			redirect(base_url('member/widtdrawal/done/'.rand(2100,8999) ),true);
			exit();
