...
/*
		...
		$key_to_find=json_encode($param);
		$key=false;
		if(!is_array($saved_session)){
		//	echo '<pre>';var_dump($saved_session);exit;
			$saved_session=array();
		}
		else{
			foreach($saved_session as $num=>$values){
				if($values['expired']<= time() ){
					//$_COOKIE['key_'.$num]=false;
					//setcookie('key_'.$num, '' );
					unset($saved_session[$num]);
				}
			}
		}
		// $_COOKIE['save_key']=$saved_session;
