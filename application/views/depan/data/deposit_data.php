...
if (function_exists('logFile')) {
    logFile('view/member/data', 'deposit_data.php', 'data');
};
ob_start();
//api_data
...
foreach ($dt as $row) {
    ...
    $row['action'].=$row['status'] == 0 ? '<a target="_blank" href="' . site_url('admin/deposit/detail/' . 
            $row['id']) . '"><input type="button" value="Detail" /></a>' : '';
...
//==============accountid
    ...
    $row['raw']->orderDeposit = '$' . number_format($row['raw']->orderDeposit, 2) . '<br/> ' .
            $currency['symbol'] . number_format($row['raw']->order1, 2) . '<br/>Rate  ' .
            $currency['symbol'] . number_format($row['raw']->rate, 2) . '<BR/>' . $currency['name'];
    $row['flowid'] = sprintf("%s%04s", date("ymd", strtotime($row['created'])), $row['id']);
    $data[] = $row;
}

$respon['data'] = $data;
$respon['raw'][] = array($userlogin, $post0, $respon['sql']);
unset($respon['sql']);
$respon['time'][] = microtime(true);
