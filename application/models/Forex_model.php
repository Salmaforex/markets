...
if (function_exists('logFile')) {
    logFile('model', 'forex_model.php', 'model');
};
...
//=================FLOW LOG
    ...
    private function flowInsertTable(){
    ...
    function flowInsert($type = '', $data = array()) {
        ...
        $dt_trans=$dt;
        $dt_trans[]=isset($data['orderDeposit'])?$data['orderDeposit']:NULL;
        $dt_trans[]=isset($data['orderWidthdrawal'])?$data['orderWidthdrawal']:NULL;
        ksort($dt_trans);
        
        log_info_table('tran_'.$type, $dt_trans);
